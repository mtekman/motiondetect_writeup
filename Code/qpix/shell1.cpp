# Shell script calling -t flag for different Image Sizes
for left in $(seq 0 50); do
   for top in $(seq 0 50); do
      for mod in $(seq 1 0.2 3); do
...
if( (t_index=args.indexOf("-t"))!=-1)
   {
       mod = args.at(t_index+1).toInt();
       offset = args.at(t_index+2).toInt();
       left = args.at(t_index+3).toInt();
       right = args.at(t_index+4).toInt();
    }
