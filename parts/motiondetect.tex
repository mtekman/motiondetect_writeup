...
It should be noted that the standard deviation in the average threshold grew as the image size grew, meaning that this was not the most accurate scale for determining a threshold. I did try setting a threshold with image size using the formula of:\\
 Thresh = 26297*ln(image_width) - 131544(+10000 as a max limiter),  giving appropriate theshold sizes of:\\
...
%DISPLAY FALSE POSITIVE IMAGES WITH LOTS OF SCATTER
...
\paragraph{In this method, a \bf{reference frame} is created by capturing a quick succesion of frames. The pixel values from these frames are added to an accumulator and then divided by the number of frames. This 'average frame' now becomes the reference image which all incoming frames are now checked against it, as shown the pesudo code bwlow
\begin{frame}[fragile]
...
\begin{frame}[fragile]
	%accumulator1 -- includes the running code as well as a CORRECTed update reference frame method
\end{frame}
...
\subpargraph{Erode, Dilate}{Open/Close}
